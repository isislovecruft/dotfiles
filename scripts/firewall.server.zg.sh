...
## ALLOW SSH (AND MOSH!)
if [[ "$SSH_PORT" != "" ]]; then
    sudo iptables -A INPUT -p tcp -m state --state NEW,ESTABLISHED,RELATED --dport $SSH_PORT -j ACCEPT
...
if [[ "$SSH_ALT_PORT" != "" ]]; then
    sudo iptables -A INPUT -p tcp -m state --state NEW,ESTABLISHED,RELATED --dport $SSH_ALT_PORT -j ACCEPT
    ## XXX the UDP might not be needed since MOSH does a handshake/keyexchange first over TCP
...
if $MOSH ; then
    ...
    sudo iptables -A OUTPUT -p udp --dport $MOSH_PORTS -j ACCEPT
fi
